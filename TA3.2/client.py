...
def move_player(steps, color):
    ...
    #Create flag variable 'updated' with value false, to see if steps have been updated
    

    for box in boxes:
        # Ony perform increment of steps if after increment its value is less then length of boxes
        if(box.cget("bg") == color):
            box.configure(bg='white')
            steps += boxes.index(box)
            # Set updated to True
            
            # Check if steps equals to len(boxes)
            
                # Set color of finishing_box to color
                
                # Set updated to false
                

    # Check if updated is true before updating color of the box
